...
def get_dist(start_lat, start_lng, dest_lat, dest_lng, c_id, c_key):
    ...
    # 거리가 너무 가까우면 계산하지 않음.
    if math.isclose(start_lng, dest_lng, rel_tol=1e-6) and math.isclose(start_lat, dest_lat, rel_tol=1e-6):
        return 0
    ...
    try:
        ...
        print(f"응답 데이터에서 예상되는 키를 찾을 수 없음: {e}, {response.text[:100]}")
        return None

    return dist

def recommend_hospital3(emergency, start_lat, start_lng, c_id, c_key):
